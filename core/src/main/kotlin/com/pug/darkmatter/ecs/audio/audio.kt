package com.pug.darkmatter.ecs.audio
    ...
    override fun play(soundAsset: SoundAsset, volume: Float) {
        when {
            ...
            soundRequests.size >= MAX_SOUND_INSTANCES -> {
                // Some devices such as android has a limitation on the maximum number of sounds can play simultaneously
                LOG.debug { "Maximum sound request reached" }
                return
            }
            else -> {
                // case when this is the first time this sound was requested to be played
                if (soundAsset.descriptor !in assets) {
                    LOG.error { "Trying t play a sound which is not loaded: $soundAsset" }
                    return
